...
class DTCoreShortcodesDefination {
	...
	function dt_sc_pricing_table_item($attrs, $content = null) {
		...
		$out = "<div class='dt-sc-pr-tb-col type1 $selected'>";
			$out .= '<div class="dt-sc-tb-header">';
				...
				elseif( $thumb && $type == 'classic' ):
					$image = wpb_getImageBySize( array( 'attach_id' => $thumb, 'thumb_size' => 'full' ));
					$image = $image['p_img_large'][0];
					$out .= '<div class="dt-sc-price" style="background-image:url('.esc_url( $image ).')">';
				endif;
	...
	function dt_sc_url($attrs, $content = null) {
		...
		if( !empty($a_href) ):
			$out  = "<div class='text-with-icon {$el_class}'>";
			$out .= "<span class='{$class}'> </span>";
		...
		$out = "<div class='dt-sc-image-caption {$type} {$class}'>";
			...
			$html_1 = "<div class='dt-sc-image-wrapper'>";
				if(!empty($image)):
					...
					if($type == 'type7')
						$html_1 .= '<div class="dt-image-with-anti-corner-frame">';
				...
				if( (!empty($icon) || !empty($iconurl)) && $type != 'type7' && $type != 'type6' ):
			...
			$html_2 = "<div class='dt-sc-image-content'>";
				...
				if( (!empty($icon) || !empty($iconurl)) && ( $type == 'type7' || $type == 'type6' ) ):
					$html_2 .= "<div class='icon-wrapper'>";
						if(!empty($icon)):
							$html_2 .= "<span class='{$icon}'> </span>";
						elseif(!empty($iconurl)):
							$image = wpb_getImageBySize( array( 'attach_id' => $iconurl, 'thumb_size' => 'full' ));
							$html_2 .= $image['thumbnail'];
						endif;	
					$html_2 .= "</div>";
				endif;
	...
	function dt_sc_team($attrs, $content = null) {
		...
		$out = "<div class='dt-sc-team {$teamstyle} {$class}'>";
			...
			if($teamstyle == 'hide-social-show-on-hover inverse-block'):
				$out .= "<div class='dt-sc-team-details hvr-block'>";
					$out .= "<h4>{$name}</h4>";
				$out .= "</div>";
			endif;	
			...
			$out .= "<div class='dt-sc-team-thumb'>";
				...
				if($teamstyle == 'type2' || $teamstyle == 'hide-social-show-on-hover inverse-block'):
	...
	function dt_sc_testimonial($attrs, $content = null) {
		...
		$image = wpb_getImageBySize( array( 'attach_id' => $image, 'thumb_size' => 'full' ));
		$image = isset( $image['thumbnail'] ) ? $image['thumbnail'] : '';
		...
		$out = '<div class="dt-sc-testimonial-wrapper">';
			$out .= "<div class='dt-sc-testimonial {$type} {$class}'>";
				...
				if($type == 'type1' || $type == 'type3' || $type == 'type5' || $type == 'type6' || $type == 'type7' || $type == 'type8')
					$out .= $img.$con;
				else
					$out .= $con.$img;
	...
	function dt_sc_twitter_tweets($attrs, $content = null) {
		...
		if($username && $consumerkey && $consumersecret && $accesstoken && $accesstokensecret) {
			...
			require_once(AAGAN_CORE_PLUGIN."/apis/twitteroauth/twitteroauth.php");
				...
				$out .= '<article id="post-'.$id.'" class="'.esc_attr($post_classes).'">';
					switch( $style ):
						...
						case 'entry-date-left':
							ob_start();
							$obj->aagan_post_date_left_style( $id, $meta );
							$out .= ob_get_clean();
						break;

						case 'entry-date-author-left':
							ob_start();
							$obj->aagan_post_date_author_left_style( $id, $meta );
							$out .= ob_get_clean();
						break;
						
						case 'entry-date-left outer-frame-border':
							ob_start();
							$obj->aagan_post_date_left_framed_border_style( $id, $meta );
							$out .= ob_get_clean();
						break;

						case 'blog-modern-style':
							ob_start();
							$obj->aagan_post_modern_style( $id, $meta );
							$out .= ob_get_clean();
						break;

						case 'bordered':
							ob_start();
							$obj->aagan_post_bordered_style( $id, $meta );
							$out .= ob_get_clean();
						break;

						case 'classic':
							ob_start();
							$obj->aagan_post_classic_style( $id, $meta );
							$out .= ob_get_clean();
						break;
						
						case 'entry-overlay-style':
							ob_start();
							$obj->aagan_post_overlay_style( $id, $meta );
							$out .= ob_get_clean();
						break;
						
						case 'overlap':
							ob_start();
							$obj->aagan_post_overlap_style( $id, $meta );
							$out .= ob_get_clean();
						break;
						
						case 'entry-center-align':
							ob_start();
							$obj->aagan_post_stripe_style( $id, $meta );
							$out .= ob_get_clean();
						break;
						
						case 'entry-fashion-style':
							ob_start();
							$obj->aagan_post_fashion_style( $id, $meta );
							$out .= ob_get_clean();
						break;
						
						case 'entry-minimal-bordered':
							ob_start();
							$obj->aagan_post_minimal_bordered_style( $id, $meta );
							$out .= ob_get_clean();
						break;
						...
						default:
							ob_start();
							$obj->aagan_post_default_style( $id, $meta );
							$out .= ob_get_clean();
						break;
	...
	function dt_sc_portfolios( $attrs, $content = null ){
		...
		$img_size = array( 
			'1' => 'full',
			'2' => 'aagan-portfolio-ii-column',
			'3' => 'aagan-portfolio-iii-column',
			'4' => 'aagan-portfolio-iv-column'
		);
	...
	function dt_sc_down_count($attrs, $content = null) {
		...
		$out .= '<div class="downcount '.$class.'" data-date="'.aagan_cs_get_option('comingsoon-launchdate', date('m/d/Y')).'" data-offset="'.aagan_cs_get_option('comingsoon-timezone', '0').'">';
	...
	function dt_sc_sociable($attrs, $content = null) {
		...
		foreach( $socials as $social ){
		   if( array_key_exists( $social, $social_opts ) ) {
			   $list .= '<li><a  class="fa fa-'.esc_attr( $social ).'" target="'.esc_attr( $target ).'" href="'.esc_attr( $social_opts[$social] ).'"></a></li>';
		   }
		}
	...
	function dt_sc_working_hours( $attrs, $content = null ){
		...
		return "<div class='dt-working-hours $class'>{$text}<ul>{$content}</ul></div>";
	}
	...
	function dt_sc_privacy_disable_google_webfonts( $attrs = array() , $content = "") {
		$content = !empty($content) ?  $content : __('Click to enable/disable google webfonts.', 'dt_framework');
	...
	function dt_sc_privacy_disable_google_maps( $attrs = array() , $content = "") {	
		$content = !empty($content) ?  $content : __('Click to enable/disable google maps.', 'dt_framework');
	...
	function dt_sc_privacy_disable_video_embeds( $attrs = array() , $content = "") {	
		$content = !empty($content) ?  $content : __('Click to enable/disable video embeds.', 'dt_framework');
