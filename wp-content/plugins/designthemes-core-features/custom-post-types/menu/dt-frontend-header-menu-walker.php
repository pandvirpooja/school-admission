       ...
       public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
            ...
            if ($item->object == 'dt_mega_menus' && $depth == 1) {
                ...
                if( !empty( $sc_custom_css ) ) {
                    ...
                    wp_add_inline_style( 'aagan-custom-inline', wp_strip_all_tags( $sc_custom_css ) );
                }

                $custom_css = get_post_meta($item->object_id, '_wpb_post_custom_css', true);
                if( !empty( $custom_css ) ) {
                    wp_enqueue_style( 'aagan-custom-inline' );
                    wp_add_inline_style( 'aagan-custom-inline', wp_strip_all_tags( $custom_css ) );
                }                
            } else {
                ...
                if ( !empty($item->icon) ) {
                 ...
                 // add image if set
                 if ( !empty($item->image) ) {
					$item_output .= '<i class="menu-item-icon menu-item-image">' . '<img src="'.$item->image.'" />' . '</i>';
                }
