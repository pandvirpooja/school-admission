...
if (! class_exists ( 'DTBaseSC' ) ) {
    ...
    class DTBaseSC {
        ...
        function dt_print_css( $css ) { 
            if( !empty( $css ) ) {
                wp_enqueue_style( 'aagan-custom-inline' );
        ...
        function dt_current_skin( $code = 'primary-color' ) {
            ...
            $color = '';
            $mode = get_theme_mod( 'use-predefined-skin', aagan_defaults('use-predefined-skin') );

            if( $mode ) {
                $skin = get_theme_mod( 'predefined-skin', aagan_defaults('predefined-skin' ) );
                $skin = aagan_skins( $skin );
                $color = $skin[$code];
            } else {
                $color = get_theme_mod( $code, aagan_defaults( $code ) );
            }
