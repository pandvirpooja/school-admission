...
if (! class_exists ( 'DTCorePlugin' )) {
	...
	class DTCorePlugin {
		...
		function __construct() {
			...
			if( ($name == 'Aagan') || ($name == 'Aagan Child') ) {
			} else {
				if( $template == 'aagan' ) {
				} else {
					return;
		...
		function dt_widgets_init() {
			register_widget('Aagan_Twitter');
			register_widget('Aagan_Flickr');
			register_widget('Aagan_Recent_Posts');
			register_widget('Aagan_Portfolio_Widget');
		}
		...
		public static function dtCorePluginActivate() {
			if( ! function_exists('aagan_cs_get_option') ){
