...
if( !class_exists( 'DTCorePrivacy' ) ) {
	...
	class DTCorePrivacy {
		...
		function dt_privacy_footer_script() {
			...
			wp_add_inline_script( 'aagan-cookie-js', "function dt_privacy_cookie_setter( cookie_name ) {
        ...
        if( cs_get_option('enable-cookie-consent') == "true" ) {
            ...
            <div class="dt-cookie-consent cookiebar-hidden dt-cookiemessage-<?php echo esc_attr($position); ?>">
	            <div class="container">
					...
					$buttons = aagan_cs_get_option('cookie-bar-buttons', array());
					$i = 0;
					$extra_info = "";
			...
			[dt_sc_privacy_video_embeds]', 'designthemes-core' ), '<br>'
							...
							if( cs_get_option('enable-custom-model-content') == "true" ) {
								$contents = aagan_cs_get_option('custom-model-tabs', array());
								$heading  = str_replace("'", "&apos;", aagan_cs_get_option('custom-model-heading', $heading));
							}
